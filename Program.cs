...
public void Task_2_11(int[,] A, int[,] B)
    ...
    public void Task_3_10(ref int[,] matrix)
    {
        ...
        int indexMaxi = max(matrix);
        int indexMini = min(matrix);
        RemoveColumns(ref matrix, indexMaxi, indexMini);
    }
    ...
    public void RemoveColumns(ref int[,] matrix, int indexMax, int indexMin)
    {
        int n = matrix.GetLength(0);
        int m = matrix.GetLength(1);
        if (indexMax == indexMin)
        {
            matrix = RemoveColumn(matrix, indexMax);
        }
        else
        {
            matrix = RemoveColumn(matrix, indexMax);
            ...
            matrix = RemoveColumn(matrix, indexMin);
        }
    }
    ...
    public int[,] RemoveColumn(int[,] matrix, int columnIndex)
    {
        int n = matrix.GetLength(0);
        int m = matrix.GetLength(1);
        if (columnIndex < 0 || columnIndex >= m) return matrix;
